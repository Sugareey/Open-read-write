...
# Задача - 1: #
def read_cookbook():
    file_path = (r'C:\Python\recipes.txt') # Используем команду os.path.join(os.getcwd()
    cook_book = {}
...
def rewrite_file(path1=None, path2=None, path3=None):
    if path1 or path2 or path3 is None:
        ...
        path2 = '2.txt'
        path3 = '3.txt'
        os.path.abspath(r'C:\Python\sorted') 
            # По аналогии с задачей 1 указал свой путь, но для общего использования можно указать просто 'sorted'
        ...
        print('Упс, что-то пошло не по плану!!')
    return
